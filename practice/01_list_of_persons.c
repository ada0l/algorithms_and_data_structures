...
void list_push(List* list, ListType* p_value)
{
    if (list->data == NULL) {
        list->capacity = 10;
        list->data
            = (ListType**)malloc(sizeof(ListType*) * list->capacity);
    }
    if (list->size >= list->capacity) {
        list->capacity = (list->capacity + 1) * 2;
        list->data = (ListType**)realloc(
            list->data, sizeof(ListType*) * list->capacity);
    }
    list->data[list->size++] = p_value;
}

size_t list_get_size(List* list) { return list->size; }
